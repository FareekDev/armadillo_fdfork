...
static int DisassembleConditionalImmediateBranchInstr(struct instruction *i,
    ...
    unsigned long imm = (sign_extendl(imm19, 19) << 2) + i->PC;
...
static int DisassembleBarrierInstr(struct instruction *i,
    ...
    if(op2 == 4 || op2 == 5){
        ...
        if(op2 == 5){
            ...
            ADD_IMM_OPERAND(out, AD_IMM_UINT, *(unsigned int *)&CRm);
        }
        else if((CRm & ~4) != 0){
            instr_s = "dsb";
...
static int DisassemblePSTATEInstr(struct instruction *i, struct ad_insn *out){
    ...
    if(op1 == 0 && (op2 == 0 || op2 == 1 || op2 == 2)){
        ...
        unsigned pstatefield = (op1 << 3) | op2;

        ADD_IMM_OPERAND(out, AD_IMM_UINT, *(unsigned int *)&pstatefield);
        ADD_IMM_OPERAND(out, AD_IMM_UINT, *(unsigned int *)&CRm);
...
static int SysOp(unsigned op1, unsigned CRn, unsigned CRm, unsigned op2){
...
static const char *at_op(unsigned encoding){
    switch(encoding){
...
static const char *dc_op(unsigned encoding){
    switch(encoding){
...
static const char *ic_op(unsigned encoding){
    switch(encoding){
...
static const char *tlbi_op(unsigned encoding){
    switch(encoding){
...
static int DisassembleSystemInstr(struct instruction *i, struct ad_insn *out){
    ...
    if(L == 0){
        if(CRn == 7 && (CRm >> 1) == 4 && SysOp(op1, 7, CRm, op2) == Sys_AT){
            ...
            unsigned encoding = (op1 << 4);
            encoding |= ((CRm & 1) << 3);
            encoding |= op2;

            ADD_IMM_OPERAND(out, AD_IMM_UINT, *(unsigned int *)&encoding);
            ...
            ADD_IMM_OPERAND(out, AD_IMM_UINT, *(unsigned int *)&op1);
            ADD_IMM_OPERAND(out, AD_IMM_UINT, *(unsigned int *)&CRn);
            ADD_IMM_OPERAND(out, AD_IMM_UINT, *(unsigned int *)&CRm);
            ADD_IMM_OPERAND(out, AD_IMM_UINT, *(unsigned int *)&op2);
...
static const char *get_sysreg(unsigned encoding){
    switch(encoding){
...
static int DisassembleSystemRegisterMoveInstr(struct instruction *i,
    ...
    int instr_id = L == 0 ? AD_INSTR_MSR : AD_INSTR_MRS;

    unsigned sreg = ((2 + o0) << 14);
    sreg |= (op1 << 11);
    sreg |= (CRn << 7);
    sreg |= (CRm << 3);
    sreg |= op2;
...
static int DisassembleTestAndBranchImmediateInstr(struct instruction *i,
    ...
    ADD_IMM_OPERAND(out, AD_IMM_UINT, *(unsigned int *)&bit_pos);
    ADD_IMM_OPERAND(out, AD_IMM_LONG, *(long *)&imm);
